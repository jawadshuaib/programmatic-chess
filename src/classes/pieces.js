...
// i.e. Rook on A3
class Piece {
  constructor (boardEl, draggable) {
    this.boardEl = boardEl;
    this.draggable = draggable !== false ? true : false;
  }
  props () {
    const pieces = [
      {
  ...
  className (color, alias) {
    ...
    const piece = this.props ().pieces.find ( prop => prop.alias === alias);    
  ...
  img (color, alias) {
    const piece = this.props().pieces.find (piece => piece.alias === alias);
    return piece.img[color];
  }

  place (color, alias, position) {
    position = position.toLowerCase ();
    const { boardEl } = this.props ();
    const img = this.img (color, alias);
...
class Pieces extends Piece {
  ...
  constructor (boardEl, playAs, draggable) {
